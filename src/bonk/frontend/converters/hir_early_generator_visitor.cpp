...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeIdentifier* node) {
    ...
    auto id = front_end.id_table.get_id(node);
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeVariableDefinition* node) {
    ...
    int variable_id = front_end.id_table.get_id(node);
    auto variable_value = std::make_unique<HIRValue>(*this);
    auto variable_type = front_end.type_table.get_type(node);
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeNumberConstant* node) {
    ...
    Type* type = front_end.type_table.get_type(node);
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeBinaryOperation* node) {
    ...
    Type* left_type = front_end.type_table.get_type(node->left.get());
    Type* right_type = front_end.type_table.get_type(node->right.get());
    ...
    if (operation_type == HIROperationType::assign) {
        ...
        HIRDataType result_type = convert_type_to_hir(front_end.type_table.get_type(node));
        ...
        result_value->set_value(id, front_end.type_table.get_type(node));
...
void bonk::HIREarlyGeneratorVisitor::compile_lazy_logic(TreeNodeBinaryOperation* node) {
    ...
    int return_lhs_label = front_end.id_table.get_unused_id();
    int calculate_rhs_label = front_end.id_table.get_unused_id();
    int end_label = front_end.id_table.get_unused_id();
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeUnaryOperation* node) {
    ...
    Type* type = front_end.type_table.get_type(node->operand.get());
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeHiveAccess* node) {
    ...
    auto hive_type = front_end.type_table.get_type(node->hive.get());
    ...
    int offset = front_end.get_hive_field_offset(hive_definition, field_index);
    ...
    auto field_type = front_end.type_table.get_type(node);
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeBonkStatement* node) {
    ...
    if (node->expression) {
        Type* type = front_end.type_table.get_type(node->expression.get());
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeLoopStatement* node) {
    ...
    int loop_start_id = front_end.id_table.get_id(node);
    int loop_end_id = front_end.id_table.get_unused_id();
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeCall* node) {
    ...
    auto definition = front_end.symbol_table.symbol_definitions[identifier].get_local().definition;
    auto identifier_type = front_end.type_table.get_type(definition);

    assert(identifier_type->kind == TypeKind::blok);

    auto block_type = (BlokType*)identifier_type;

    int label_id = front_end.id_table.get_id(definition);
    ...
    for (auto& parameter : block_type->parameters) {
        ...
        front_end.compiler.error().at(node->source_position)
...
void bonk::HIREarlyGeneratorVisitor::visit(bonk::TreeNodeNull* node) {
    ...
    auto null_type = front_end.type_table.get_type(node);
